...
app.post("/addstoryforapp",auth,async(req,res)=>{
  try{
    ...
    const addtomystory=await Register.findOneAndUpdate({email:useremail},{$push:{stories:{
      img_depressor:img_depressor,
      img_reliever:photoo,
      email:useremail,
      story:story,
...
app.post("/deletechats",auth,async (req,res)=>{
  try{
    ...
    const email=req.rootUser.email;
    if(type=="receive"){
      const newresult=await Register.findOneAndUpdate( { email: email },  { $pull: { mychats:{_id:chatid,type:"received",message:chat}  } } );
    }
    else{
    ...
    const newresult=await Register.findOneAndUpdate( { email: email },  { $pull: { mychats:  {_id:chatid,type:"send",message:chat}  } } );
    }
...
io.on("connection", (socket) => {
  ...
  socket.on("chat message", (data) => {
    ...
    io.to(data.room).emit("chat message", datasento);
  });


  //  socket.on("chat message", msg => {
  //   console.log(msg);
  //  io.to(100).emit("chat message", msg);
  // });
