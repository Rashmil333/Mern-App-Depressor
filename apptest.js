...
app.post("/sahyog_memory",async(req,res)=>{
  try{
    ...
    const result= await Register.findOneAndUpdate({email:'allusers@gmail.com'},{$push:{
      memory:{
        key:key,
        value:value
      }
...
app.post("/placeorder",auth,async(req,res)=>{
  try{
    ...
    console.log("the placeoreder is "+product_name_n+quantity_);
    const cost_n=req.body.cost;
...
app.post("/update_password",auth,async(req,res)=>{
  try{
    ...
    const secretpass=await bcrypt.hash(pass,10);
   const result=await Register.findOneAndUpdate( { email: email },  { $set: { 
             password:secretpass,
             cpassword:repass
           } 
        });
    console.log("Yes i am working"+"and pass repas is"+pass+"/"+repass)
    res.status(200).send("testing");
  }
  catch(err){
    console.log(err)
